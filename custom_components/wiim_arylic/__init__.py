...
# ---------------------------------------------------------------------------
# Arylic pywiim patch — shadow specific pywiim modules with local patched versions.
# ---------------------------------------------------------------------------
import sys
...
_PYWIIM_LOCAL = Path(__file__).parent / "pywiim_local"

# 1. Force Python to load the real, underlying packages FIRST.
# This builds the proper namespace so relative imports have a parent to attach to.
try:
    import pywiim
    import pywiim.api
    import pywiim.player
    import pywiim.upnp
except ImportError as e:
    _LOGGER.warning("Could not load base pywiim packages for patching: %s", e)

_PATCHES = [
    ("pywiim.profiles",        _PYWIIM_LOCAL / "profiles.py"),
    ("pywiim.api.playback",    _PYWIIM_LOCAL / "api" / "playback.py"),
    ("pywiim.player.coverart", _PYWIIM_LOCAL / "player" / "coverart.py"),
    ("pywiim.upnp.client",     _PYWIIM_LOCAL / "upnp" / "client.py"),
]

for _target, _path in _PATCHES:
    if not _path.exists():
        continue
        
    try:
        # 2. Create the spec using the FULL target name. 
        # Python will automatically set __package__ to 'pywiim' based on the name.
        _spec = importlib.util.spec_from_file_location(_target, str(_path))
        if _spec and _spec.loader:
            _mod = importlib.util.module_from_spec(_spec)
            
            # 3. Inject it into sys.modules BEFORE executing to handle circular imports
            sys.modules[_target] = _mod
            
            # 4. Execute the module. Relative imports (from .models) will now 
            # safely resolve against the real pywiim package we loaded in step 1.
            _spec.loader.exec_module(_mod)
            
    except Exception as _patch_err:
        _LOGGER.warning("Arylic patch failed for %s: %s", _target, _patch_err)
        # Rollback the specific patch if it fails
