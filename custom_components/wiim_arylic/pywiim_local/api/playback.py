...
class PlaybackAPI:
    ...
    async def resume(self) -> None:
        ...
        Arylic A31 devices do not recognise setPlayerCmd:resume.
        We try the HTTP command first; if it fails (or returns an
        "unknown command" response) we fall back to the UPnP AVTransport
        Play SOAP action, which works reliably on Arylic hardware.

        Raises:
            WiiMError: If both HTTP and UPnP fallback fail.
        """
        # Try standard HTTP resume first
        try:
            result = await self._request(API_ENDPOINT_RESUME)  # type: ignore[attr-defined]
            # Arylic returns the literal string "unknown command" on unsupported calls
            raw = getattr(result, "raw", None)
            if raw and str(raw).strip().lower().startswith("unknown command"):
                raise Exception("unknown command")
            return
        except Exception as _http_err:
            _LOGGER.debug(
                "HTTP resume failed (%s), trying UPnP AVTransport Play fallback",
                _http_err,
            )

        # UPnP fallback: AVTransport Play action works on Arylic A31
        upnp = getattr(self, "_upnp_client", None)  # type: ignore[attr-defined]
        if upnp is not None:
            try:
                await upnp.async_call_action(
                    "av_transport",
                    "Play",
                    {"InstanceID": 0, "Speed": "1"},
                )
                _LOGGER.debug("UPnP AVTransport Play fallback succeeded for resume")
                return
            except Exception as _upnp_err:
                _LOGGER.warning("UPnP Play fallback also failed: %s", _upnp_err)

        _LOGGER.warning("resume() failed: HTTP unsupported and no UPnP client available")
    ...
    async def next_track(self) -> None:
        ...
        Arylic devices sometimes silently ignore setPlayerCmd:next on
        streaming sources (e.g. Spotify Connect). Falls back to UPnP
        AVTransport Next SOAP action which is reliable on Arylic hardware.

        Raises:
            WiiMError: If the request fails.
        """
        try:
            result = await self._request(API_ENDPOINT_NEXT)  # type: ignore[attr-defined]
            raw = getattr(result, "raw", None)
            if raw and str(raw).strip().lower().startswith("unknown command"):
                raise Exception("unknown command")
            return
        except Exception as _http_err:
            _LOGGER.debug("HTTP next_track failed (%s), trying UPnP fallback", _http_err)
        ...
        if upnp is not None:
            try:
                ...
                _LOGGER.debug("UPnP Next fallback succeeded")
                return
            except Exception as _upnp_err:
                _LOGGER.warning("UPnP Next fallback also failed: %s", _upnp_err)

        _LOGGER.warning("next_track() failed: HTTP unsupported and no UPnP client available")

    async def previous_track(self) -> None:
        """Skip to previous track.

        Same Arylic fallback pattern as next_track(): HTTP may be ignored
        on streaming sources, UPnP Previous SOAP action is reliable.

        Raises:
            WiiMError: If the request fails.
        """
        try:
            result = await self._request(API_ENDPOINT_PREV)  # type: ignore[attr-defined]
            raw = getattr(result, "raw", None)
            if raw and str(raw).strip().lower().startswith("unknown command"):
                raise Exception("unknown command")
            return
        except Exception as _http_err:
            _LOGGER.debug("HTTP previous_track failed (%s), trying UPnP fallback", _http_err)
        ...
        if upnp is not None:
            try:
                ...
                _LOGGER.debug("UPnP Previous fallback succeeded")
                return
            except Exception as _upnp_err:
                _LOGGER.warning("UPnP Previous fallback also failed: %s", _upnp_err)

        _LOGGER.warning("previous_track() failed: HTTP unsupported and no UPnP client available")
