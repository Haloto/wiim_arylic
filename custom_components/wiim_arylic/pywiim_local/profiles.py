...
PROFILE_ARYLIC = DeviceProfile(
    ...
    # ARYLIC PATCH: Use UPnP for position/duration so Music Assistant gets
    # live progress updates. HTTP polling is too slow (~10s intervals);
    # UPnP events fire in real time. Everything else stays on HTTP.
    state_sources=StateSourceConfig(
        position="upnp",
        duration="upnp",
    ),
