...
class UpnpClient:
    ...
    async def get_info_ex(self) -> dict[str, Any]:
        """Fetch extended device state via GetInfoEx UPnP action.

        GetInfoEx is a LinkPlay/Arylic-specific AVTransport action that
        returns everything in a single call. This is more efficient than
        calling GetTransportInfo + GetPositionInfo + GetMediaInfo separately.

        Unlike the standard GetPositionInfo, GetInfoEx also returns:
        - CurrentTransportState (PLAYING, PAUSED_PLAYBACK, STOPPED)
        - CurrentVolume and CurrentChannel
        - LoopMode (0=loop all, 1=loop one, 2=shuffle)
        - SlaveList (JSON string of multiroom group members with IP/UUID/volume)
        - TrackSource (e.g. "iPhone_RemoteLocal", "Spotify")
        - SpotifyActive flag
        - BatteryFlag and BatteryPercent
        - TrackMetaData: DIDL-Lite XML containing title, artist, album,
          and crucially <upnp:albumArtURI> with the artwork URL

        Full response fields (from GetInfoEx.sh documentation):
            CurrentTransportState, CurrentTransportStatus, CurrentSpeed,
            Track, TrackDuration, TrackMetaData (DIDL-Lite XML),
            TrackURI, RelTime, AbsTime, RelCount, AbsCount,
            CurrentVolume, CurrentChannel, LoopMode,
            SlaveList (JSON), PlayMedium, TrackSource,
            InternetAccess, SpotifyActive, BatteryFlag, BatteryPercent

        Returns:
            Dictionary with all of the above fields. Missing fields
            will simply be absent from the dict.

        Raises:
            UpnpError: If AVTransport service is not available or action fails.
                       Callers should catch this — not all firmware versions
                       support GetInfoEx, in which case fall back to
                       get_position_info() + get_transport_info().
        """
        if not self._av_transport_service:
            raise UpnpError("AVTransport service not available")
        ...
        try:
            ...
            _LOGGER.debug("GetInfoEx result for %s: %s", self.host, result)
            return result
        except Exception as err:
            _LOGGER.debug(
                "GetInfoEx failed for %s (firmware may not support it): %s",
                self.host,
                err,
            )
